...
class TrajGRU(nn.Module):
    def __init__(self, input_size, window_in, window_out, encoder_params, decoder_params, device, num_classes=4):
        ...
        self.final_conv = nn.Conv2d(
            in_channels=decoder_params['hidden_dim'], 
    ...
    def forward(self, x, hidden=None):
        ...
        # 3. 解码器前向传播，得到输出特征
        # dec_output 形状通常为: [batch, window_out, hidden_dim, height, width]
        dec_output = self.__forward_block(decoder_input, cur_states[:, -1], 'decoder')

        # 4. 去除多余的时间维度 (提取唯一预测的那个时间步)
        # 将形状从 [B, 1, C, H, W] 变成 [B, C, H, W] 以匹配 nn.Conv2d
        if dec_output.dim() == 5:
            last_hidden = dec_output[:, 0, :, :, :]  # 取出 window_out 的第0个（也是唯一一个）元素
        else:
            last_hidden = dec_output

        # 5. 通过 1x1 卷积映射到 4 个干旱等级类别
        out = self.final_conv(last_hidden) 
